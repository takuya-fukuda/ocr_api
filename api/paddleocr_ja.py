...
def get_args():
    ...
    parser.add_argument(
        "--det_model",
        type=str,
        default='./api/ppocr_onnx/model/det_model/ch_PP-OCRv3_det_infer.onnx',
    )
    parser.add_argument(
        "--rec_model",
        type=str,
        default='./api/ppocr_onnx/model/rec_model/japan_PP-OCRv3_rec_infer.onnx',
    )
    parser.add_argument(
        "--rec_char_dict",
        type=str,
        default='./api/ppocr_onnx/ppocr/utils/dict/japan_dict.txt',
    )
    parser.add_argument(
        "--cls_model",
        type=str,
        default='./api/ppocr_onnx/model/cls_model/ch_ppocr_mobile_v2.0_cls_infer.onnx',
    )
...
def paddle_main(image_path):
    ...
    if image_path is not None:
        ...
        for bbox, text in zip(dt_boxes, rec_res):
            #return(text[0])
            ocr_result.append(text[0])
        return ocr_result
            #ocr_info = {"text": text[0], "bbox": bbox}
            #ocr_result.append(ocr_info)
            #return text[0]
        #カスタム処理終了
        ...
        #表示方法を変更（追加で入れた処理）
        #cv2.namedWindow('PaddleOCR ONNX Sample', cv2.WINDOW_NORMAL)
    ...
    # print(f"Total execution time: {elapsed_time:.2f} seconds")  # 実行時間を表示

# def draw_info(image, dt_boxes, rec_res, time_dict):
#     for bbox, text in zip(dt_boxes, rec_res):
#         # 文字をprintで表示
#         print(f"OCR結果: {text[0]}")
#         # 文字と座標をprintで表示
#         #print(f"OCR結果: {text[0]} 座標: {bbox}")
...
#         #Flaskようにパス編集
#         font_path = './api/cvdrawtext/font/LINE_Seed_JP/Desktop/OTF/LINESeedJP_OTF_Rg.otf'
